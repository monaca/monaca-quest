...
},{"1":1,"4":4,"6":6,"7":7,"8":8,"fs":"fs","path":"path"}],3:[function(require,module,exports){
...
},{"3":3,"fs":"fs","path":"path"}],8:[function(require,module,exports){
var JSON_FILES, _, enumAllAssets, enumFilesRecur, enumIncludeFiles, enumSystemFiles, excludeAssets, fs, ids2array, makeUniqueAssets, path, pickAudioName, readAnimationIds, readAnimations, readAssets, readBattleBacks1, readBattleBacks2, readBgm, readBgs, readCharacters, readEnemies, readFaces, readJson, readJsons, readMe, readParallaxes, readPictures, readSe, readSvActors, readSvEnemies, readTilesetIds, readTilesets, readVideos, resolveAssets, resolveFiles, search, searchCommand,
  slice = [].slice;
...
readJsons = function(home, files) {
  ...
  // console.log(result.maps[1]); // naito 
  return result;
};

readAssets = function(json) {
  return {
...
pickAudioName = function(json) {
  return search(json, '[*].name');
};
...
readVideos = function(json) {
  return searchCommand(json, 261, 0);
};

readAnimations = function(json) {
  return ids2array(json.Animations);
};

readAnimationIds = function(json) {
...
readBattleBacks1 = function(json) {
...
readBattleBacks2 = function(json) {
...
readParallaxes = function(json) {
  return searchCommand(json, 284, 0);
};

readPictures = function(json) {
  return searchCommand(json, 231, 1);
};

readSvActors = function(json) {
  if (json.System.optSideView) {
...
readTilesets = function(json) {
  return ids2array(json.Tilesets);
};

readTilesetIds = function(json) {
...
makeUniqueAssets = function(assets) {
...
resolveFiles = function(home, assets) {
...
enumAllAssets = function(home) {
  return _.flattenDeep(enumFilesRecur(home));
};

enumFilesRecur = function(dir) {
  var file, fullPath, i, len, ref, results;
  ...
  for (i = 0, len = ref.length; i < len; i++) {
    ...
    if (fs.lstatSync(fullPath).isDirectory()) {
      results.push(enumFilesRecur(fullPath));
    } else {
...
enumSystemFiles = function(home) {
  var file, fullPath;
  return _.flattenDeep(_.compact((function() {
    var i, len, ref, results;
    ref = fs.readdirSync(home);
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      file = ref[i];
